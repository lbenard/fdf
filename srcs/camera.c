...
/*   Updated: 2019/01/17 20:42:04 by lbenard          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_camera	*new_camera(void)
{
	...
	ret->rotation = ft_vec2f(PI / 2, 0.0f);
	return (ret);
}
...
{
  delta.x *= cos(self->rotation.y);
  delta.y *= sin(self->rotation.x);
  delta.z *= -cos(self->rotation.y) * cos(self->rotation.x);
